...
export default function Home() {
  ...
  useEffect(() => {
    ...
    API.get('/settings/whatsapp').then(res => {
      if (res.data.value) setWhatsapp(res.data.value);
    });
  }, []);
  ...
  return (
    <div className="page" style={{ maxWidth: 480, margin: '0 auto' }}>
      ...
      <div style={{
        ...
        borderBottom: '1px solid #e8ddd4', marginBottom: 36
      }}>
        <img src="/logo.png" alt="ND Outfits"
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            {categories.map((cat, i) => (
              <Link key={cat.name} to={`/category/${encodeURIComponent(cat.name)}`}>
                <div style={{
                  borderRadius: 4, overflow: 'hidden', border: '1px solid #e8ddd4',
                ...
                >
                  <div style={{ position: 'relative', paddingTop: '100%', background: '#f5ede4', overflow: 'hidden' }}>
                    {cat.imageUrl ? (
                      ...
                      <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #f5ede4, #edddd0)' }}>
                        <span style={{ fontSize: 36 }}>👗</span>
                      </div>
                    )}
                    <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(58,46,39,0.55) 0%, transparent 60%)' }} />
                    <div style={{ position: 'absolute', bottom: 12, left: 0, right: 0, textAlign: 'center' }}>
                      <div style={{ fontFamily: 'Cormorant Garamond', fontSize: 17, color: '#fff', fontStyle: 'italic', letterSpacing: 1, textShadow: '0 1px 8px rgba(0,0,0,0.3)' }}>
                        {cat.name}
                      </div>
                    </div>
                  </div>
